...
func main() {
	...
	for {
		...
		if counter%6 == 0 { // Every 6th iteration, add a test accommodation
			_, err := db.Exec(`
				INSERT INTO accommodations (name, description, source_website, external_id, verification_status, created_at) 
				VALUES ($1, $2, $3, $4, $5, $6)`,
				fmt.Sprintf("OLX Rental #%d", counter),
				fmt.Sprintf("A rental property listed on OLX, iteration %d", counter),
				models.SourceWebsiteOLX,
				fmt.Sprintf("olx_%d", counter),
